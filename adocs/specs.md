...
| `--step unclaim <id>` | `plan_current/` → `plan_todo/`, the inverse of `start`: clears `author:` and nothing else. Refuses a step that is not claimed, one in `plan_done/`, one carrying a `done:` stamp, an occupied destination, a `paused_by` (routed to the command that clears it), and a step another `plan_current/` step declares `blocks:`. A blocking child may be unclaimed; the parent it leaves paused is named |
