...
| `--step done <id> --stamp TEXT` | complete: preconditions, `test_command` gate, stamped move to `plan_done/`, parent unpause, prune `plan.md` to the last 5 done entries; `--stamp` is required free text — multi-line accepted, written as indented continuations, and refused when it contains a blank line, since a field ends at the first blank line and the continuations below it are dropped |
