...
| `--step unpause <id>` | clear a `paused_by` that names a step in no plan directory. Refuses when the pauser exists — complete that one instead, which unpauses the parent on its way out |
