...
| `--step unclaim <id>` | put a claim down: move a step from `plan_current/` back to `plan_todo/` and clear the `author:` that `--step start` stamped, because the author *is* the claim. Everything else in the file is left alone. Refused for a step that was never claimed, one in `plan_done/`, one already carrying a `done:` stamp, and one with a `paused_by` — clear the pause first, and the refusal names which command does it. A blocking child *can* be unclaimed, otherwise a claimed stack could never be put down; the parent it leaves paused is named on the way out, and `--step start` on the child picks it back up |
