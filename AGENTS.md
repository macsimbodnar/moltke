...
and the not-yet-merged side takes the next free id — file and `plan.md`
entry — before merging, noted in the merge commit; two files claiming one id
is worse.
...
- TESTS: no automated suite — the product is markdown; each `done:` stamp says
  what was verified by hand (usually: dogfooding the skills on a scratch
  repository).
...
- AUDIT: full audits on demand only.
- DEPS: none — the plugin ships markdown only.
