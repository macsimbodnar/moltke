...
agents/adversarial_reviewer.md   auditor: reads anything, writes only project/audit/
...
project/                         moltke's own workflow state (it uses itself)
```

`project/` is not part of the plugin's behaviour. It is this repository's own
plan, decisions, testing ledger, and worklog, because moltke is the first
project to use moltke. Read `project/specs.md` before changing anything.
...
110 tests, no skips. A test whose precondition is genuinely absent skips with a
...
change in `project/specs.md` and `MANUAL.md` in the same commit:
...
- Work moves through `project/plan_todo` → `plan_current` → `plan_done`, driven
...
- `testing.md` rows are added with the feature. `--step done` refuses without
  one.
- Decisions go in `project/decisions.md` before or alongside the change, with
